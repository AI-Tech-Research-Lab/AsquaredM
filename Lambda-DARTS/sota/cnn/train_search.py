...
import numpy as np
import torch
import sota.cnn.utils as utils
import logging
import argparse
...
from sota.cnn.architect import Architect
...
# Our arguments
parser.add_argument('--corr_regularization', type=str, default='none', choices=['none', 'corr', 'signcorr'], help='Add correlation regularization term')
...
if args.wandb:
    wandb.init(
        ...
        project=f"LAMBDADARTS-{args.dataset}",
        name=f"LAMBDADARTS-REGTYPE{args.corr_regularization}-SEED{args.seed}",
...
def main():
    ...
    model = Network(args.init_channels, n_classes, args.layers, criterion, spaces_dict[args.search_space], epsilon_0=args.epsilon_0, lambda_=args.lambda_, corr_type=args.corr_regularization)
    model = model.cuda()
    ...
    for epoch in range(args.epochs):
        ...
        # training
        train_acc, train_obj, train_corr, valid_corr = train(train_queue, valid_queue, model, architect, criterion, optimizer, lr)
        scheduler.step()
...
def train(train_queue, valid_queue, model, architect, criterion, optimizer, lr):
    ...
    for step, (input, target) in enumerate(train_queue):
        ...
        if args.corr_regularization != 'none':
            ...
            forward_grads = torch.autograd.grad(criterion(model(input, pert=u, updateType='weight'), target), model.parameters())
            backward_grads = torch.autograd.grad(criterion(model(input, pert=[-u_ for u_ in u], updateType='weight'), target), model.parameters())
